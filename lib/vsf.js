...
function vsf(configPath, charset) {
    ...
    (config['freeMarker'] || []).forEach((item) => {
        item.data = (typeof item.data === 'string' ? _.jsonFileValue(item.data, charset) : item.data);
    });
    (config['GET'] || []).forEach((item) => {
        item.input = (typeof item.input === 'string' ? _.jsonFileValue(item.input, charset) : item.input);
        item.output = (typeof item.output === 'string' ? _.jsonFileValue(item.output, charset) : item.output);
    });
    (config['POST'] || []).forEach((item) => {
        item.input = (typeof item.input === 'string' ? _.jsonFileValue(item.input, charset) : item.input);
        item.output = (typeof item.output === 'string' ? _.jsonFileValue(item.output, charset) : item.output);
    });
